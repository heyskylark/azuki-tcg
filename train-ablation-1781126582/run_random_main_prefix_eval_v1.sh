#!/usr/bin/env bash
# Stage 3 strength, card exposure, deck composition, and causal hybrid evaluation.
set -euo pipefail
...
HYBRID_GAMES=384
HYBRID_SHARDS=${RANDOM_PREFIX_HYBRID_SHARDS:-12}
REFERENCE_MAX_STEPS=${RANDOM_PREFIX_REFERENCE_MAX_STEPS:-1200}
...
OPPORTUNITY_SHARDS=${RANDOM_PREFIX_OPPORTUNITY_SHARDS:-12}
...
if [[ ! -f "$RESULT_ROOT/LADDER_TRAIN_DONE" ]]; then
  ...
  while [[ ! -f "$RESULT_ROOT/LADDER_TRAIN_DONE" ]]; do
    if [[ -f "$RESULT_ROOT/control_no_prefix/SPS_GUARD_FAILED" ]] ||
       [[ -f "$RESULT_ROOT/random_main_prefix/SPS_GUARD_FAILED" ]] ||
       [[ -f "$RESULT_ROOT/control_no_prefix/STARTUP_INVARIANTS_FAILED" ]] ||
       [[ -f "$RESULT_ROOT/random_main_prefix/STARTUP_INVARIANTS_FAILED" ]]; then
...
control_no_prefix_run=$(<"$RESULT_ROOT/control_no_prefix/run_dir.txt")
candidate_run=$(<"$RESULT_ROOT/random_main_prefix/run_dir.txt")

checkpoint_for() {
  local arm=$1 epoch=$2
  case "$arm" in
    parent) printf '%s\n' "$PARENT_MODEL" ;;
    control_no_prefix) printf '%s/model_azuki_local_%06d.pt\n' "$control_no_prefix_run" "$epoch" ;;
    random_main_prefix) printf '%s/model_azuki_local_%06d.pt\n' "$candidate_run" "$epoch" ;;
...
mapfile -t EVAL_EPOCHS < <(
  .venv/bin/python - "$control_no_prefix_run" "$candidate_run" "$PARENT_EPOCH" "$TARGET_EPOCH" \
...
control_no_prefix = Path(sys.argv[1])
...
available = sorted(epochs(control_no_prefix).intersection(epochs(candidate)))
...
run_reference_panel() {
  ...
  if [[ ! -f "$output" ]]; then
    ...
    .venv/bin/python python/src/native_reference_eval.py \
      ...
      --candidate-label "$label" --opponent-label stage3_parent \
...
sha256sum \
  ...
  train-ablation-1781126582/random_main_prefix_ladder_report.py \
...
for epoch in "${EVAL_EPOCHS[@]}"; do
  ...
  mkdir -p "$epoch_dir/control_no_prefix" "$epoch_dir/random_main_prefix"
  control_no_prefix_checkpoint=$(checkpoint_for control_no_prefix "$epoch")
  candidate_checkpoint=$(checkpoint_for random_main_prefix "$epoch")
  [[ -f "$control_no_prefix_checkpoint" && -f "$candidate_checkpoint" ]]

  run_uniform_panel "$candidate_checkpoint" "$control_no_prefix_checkpoint" \
    "prefix_p$epoch" "control_no_prefix_p$epoch" "$epoch_dir/prefix_vs_control_no_prefix.json"
  run_uniform_panel "$candidate_checkpoint" "$PARENT_MODEL" \
    "prefix_p$epoch" stage3_parent "$epoch_dir/prefix_vs_parent.json"
  run_uniform_panel "$control_no_prefix_checkpoint" "$PARENT_MODEL" \
    "control_no_prefix_p$epoch" stage3_parent "$epoch_dir/control_no_prefix_vs_parent.json"
  run_reference_panel "$candidate_checkpoint" "prefix_p$epoch" \
    "$epoch_dir/random_main_prefix/heldout.json"
  run_reference_panel "$control_no_prefix_checkpoint" "control_no_prefix_p$epoch" \
    "$epoch_dir/control_no_prefix/heldout.json"
  run_context_decks "$candidate_checkpoint" "$epoch_dir/random_main_prefix"
  run_context_decks "$control_no_prefix_checkpoint" "$epoch_dir/control_no_prefix"
  run_context_kl "$candidate_checkpoint" "$epoch_dir/random_main_prefix"
  run_context_kl "$control_no_prefix_checkpoint" "$epoch_dir/control_no_prefix"
done
...
for arm in control_no_prefix random_main_prefix; do
...
PYTHONPATH=python/src:train-ablation-1781126582 \
.venv/bin/python train-ablation-1781126582/random_main_prefix_ladder_report.py \
