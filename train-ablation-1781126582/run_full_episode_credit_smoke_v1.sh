#!/usr/bin/env bash
# Stage 2 integrity gate and throughput diagnostic for exact retained-row credit.
set -euo pipefail

ROOT=/home/skylark/git/azuki-tcg
cd "$ROOT"

CAMPAIGN=${FULL_CREDIT_SMOKE_CAMPAIGN:-full_episode_credit_smoke_labelfix_v1}
...
UPDATES=${FULL_CREDIT_SMOKE_UPDATES:-48}
STEADY_UPDATES=${FULL_CREDIT_SMOKE_STEADY_UPDATES:-24}
PEAK_LR=${FULL_CREDIT_SMOKE_PEAK_LR:-0.000003}
...
if (( TARGET_EPOCH % CREDIT_UPDATE_INTERVAL != 0 )); then
  echo "[$CAMPAIGN] target epoch must close a credit update window" >&2
  exit 1
fi
...
run_arm() {
  ...
  if [[ "$arm" == full_episode_credit ]] &&
     ! grep -q '\[draft-episode-credit\] enabled:' "$live_log"; then
    echo "[$CAMPAIGN] candidate did not enable full-episode credit" >&2
    exit 1
  fi
...
sha256sum \
  ...
  train-ablation-1781126582/run_full_episode_credit_smoke_v1.sh \
  > "$RESULT_ROOT/runtime_sha256.txt"
git rev-parse HEAD > "$RESULT_ROOT/git_head.txt"
printf 'campaign=%s\nparent_epoch=%s\ntarget_epoch=%s\ncredit_coef=%s\nbatch_drafts=%s\nupdate_interval=%s\n' \
  "$CAMPAIGN" "$PARENT_EPOCH" "$TARGET_EPOCH" "$CREDIT_COEF" \
  "$CREDIT_BATCH_DRAFTS" "$CREDIT_UPDATE_INTERVAL" \
  > "$RESULT_ROOT/campaign_config.txt"

env PYTHONPATH=build/python/src:python/src \
  .venv/bin/python train-ablation-1781126582/draft_terminal_credit_preflight.py \
    --config python/config/azuki_deckbuild_native_3090.ini \
    --checkpoint "$PARENT_MODEL" \
    --out-dir "$RESULT_ROOT/terminal_impulse" \
    --device cuda --uniform-assignment \
    > "$RESULT_ROOT/terminal_impulse.log" 2>&1

run_arm control 0
run_arm full_episode_credit "$CREDIT_COEF"
...
def summarize(arm):
  ...
  return {
    ...
    "target_mean_min": min(vals("losses/draft_episode_credit_target_mean"), default=None),
    "target_mean_max": max(vals("losses/draft_episode_credit_target_mean"), default=None),
    "q_examples": {
...
candidate = summarize("full_episode_credit")
...
performance = (
  ...
  and candidate["steady_interval_sps_median"] >= hard_floor
)
payload = {
  "schema_version": 1,
