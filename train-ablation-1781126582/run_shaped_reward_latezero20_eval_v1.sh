#!/usr/bin/env bash
# Post-training trajectory evaluation for shaped_reward_latezero20_v1.
set -euo pipefail
...
run_opportunity_eval() {
  ...
  if [[ "$arm" == late_zero ]] && (( epoch >= 7200 )); then
    effective_scale=0
  fi
...
if [[ ! -f "$EVAL_ROOT/runtime_sha256.txt" ]]; then
  sha256sum \
    ...
    train-ablation-1781126582/shaped_reward_latezero20_report.py \
    train-ablation-1781126582/run_shaped_reward_latezero20_eval_v1.sh \
    > "$EVAL_ROOT/runtime_sha256.txt"
fi

for epoch in "${trajectory_epochs[@]}"; do
  fixed_checkpoint=$(checkpoint_for fixed_floor "$epoch")
  late_checkpoint=$(checkpoint_for late_zero "$epoch")
  [[ -f "$fixed_checkpoint" && -f "$late_checkpoint" ]]
  epoch_root="$EVAL_ROOT/p$epoch"
  mkdir -p "$epoch_root"

  run_h2h "$late_checkpoint" "$fixed_checkpoint" \
    "late_zero_p${epoch}" "fixed_floor_p${epoch}" "$epoch_root/h2h_late_vs_floor.json"
done
  ...
  for arm in "${arms[@]}"; do
    ...
    if [[ ! -f "$arm_root/decks.json" ]]; then
      ...
      .venv/bin/python train-ablation-1781126582/dump_gate_decks.py \
        --config python/config/azuki_deckbuild_3090.ini \
        --checkpoint "$checkpoint" --episodes 16 --device cuda \
...
PYTHONPATH=python/src:train-ablation-1781126582 \
.venv/bin/python train-ablation-1781126582/shaped_reward_latezero20_report.py \
