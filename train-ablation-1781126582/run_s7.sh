#!/bin/bash
# s7combo45: 45M confirmation of the auxvd recipe (first nonzero KL at 15M).
#   auxv1  : vboot 0.05
...
export AZK_DRAFT_AUX_ANNEAL=1
export AZK_XGATE_MASK=1
run_arm s7combo45 0.05 2.0


echo "[auxmx] $(date +%F_%T) MATRIX DONE"

# per-checkpoint trajectories (KL growth is the question)
for CKPT in $(ls experiments/azuki_local_s7combo45_*/model_azuki_local_*.pt 2>/dev/null | sort); do
  EP=$(basename "$CKPT" | grep -oE '[0-9]+' | tail -1)
  JSON="$RESULTS/run45_s7combo45/gate_kl_ep${EP}.json"
  mkdir -p "$RESULTS/run45_s7combo45"
  [ -f "$JSON" ] && continue
  OMP_NUM_THREADS=4 PYTHONPATH=build/python/src:python/src:train-ablation-1781126582 \
  .venv/bin/python train-ablation-1781126582/probe_gate_kl.py \
    --checkpoint "$CKPT" --episodes 8 --device cpu --json "$JSON" > /dev/null 2>&1 || true
  echo "[s7combo45] $(date +%F_%T) gate_kl ep$EP"
done
for CKPT in $(ls experiments/azuki_local_s7combo45_*/model_azuki_local_*.pt 2>/dev/null | sort); do
  EP=$(basename "$CKPT" | grep -oE '[0-9]+' | tail -1)
  JSON="$RESULTS/run45_s7combo45/critic_ep${EP}.json"
  [ -f "$JSON" ] && continue
  OMP_NUM_THREADS=4 PYTHONPATH=build/python/src:python/src:train-ablation-1781126582 \
  .venv/bin/python train-ablation-1781126582/probe_critic_gate.py \
    --checkpoint "$CKPT" --episodes 6 --device cpu --json "$JSON" > /dev/null 2>&1 || true
  echo "[s7combo45] $(date +%F_%T) critic ep$EP"
done
echo "[s7combo45] $(date +%F_%T) ALL DONE"
