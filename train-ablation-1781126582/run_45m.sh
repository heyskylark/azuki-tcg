#!/bin/bash
# 45M confirmation runs (launch ONLY on a positive critic-sensitivity verdict):
#   combo45 : anneal + gate-id + pick-eps 0.02 + sibling-matchup oversampling 0.35
#             + league keep 6/4/3  (the distributed-run recipe candidate)
#   anneal45: anneal-only control at the same length/league (isolates "longer alone")
# Then: draft-vs-ref eval (192 eps) per arm + KL & critic trajectories per checkpoint.
# ~5.5-6h/arm at ~2.5k SPS steady + ~2.5h probes => overnight chain.
# Detach with: setsid nohup bash train-ablation-1781126582/run_45m.sh \
#   > /tmp/run45m_chain.log 2>&1 < /dev/null &
set -u
...
# probe TAG PROBE_SCRIPT OUT_PREFIX EPISODES -> per-checkpoint trajectory
trajectory() {
...
# --- arm 2: anneal-only control (same length, same league) ---
launch anneal45
draftref anneal45
