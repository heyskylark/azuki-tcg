#!/bin/bash
# Post-anneal45 continuation: waits for the running anneal45 trainer to finish,
# then draftref(anneal45) -> resume combo45 from ep600 (reset-start probe now
# skipped on native deckbuild; fingerprint bypasses + pinned anneal tail) ->
# draftref(combo45) -> per-checkpoint KL + critic trajectories for both arms.
# Detach with: setsid nohup bash train-ablation-1781126582/run_after_anneal45.sh \
#   > /tmp/run45m_chain6.log 2>&1 < /dev/null &
set -u
...
# --- wait for the running anneal45 trainer (started by chain5) ---
echo "[chain] $(date +%F_%T) waiting for running anneal45 trainer to finish"
while pgrep -f "python python/src/train[.]py .*--tag anneal45" > /dev/null; do
  sleep 60
done
echo "[chain] $(date +%F_%T) anneal45 trainer gone; continuing"
draftref anneal45

# --- resume combo45 from ep600 ---
...
unset AZK_RESUME_ALLOW_BINDING_MISMATCH AZK_RESUME_ALLOW_SOURCE_DRIFT AZK_RESUME_KEEP_CURRENT_SCHEDULE_ENV \
      AZK_REWARD_SHAPING_ANNEAL AZK_REWARD_SHAPING_ANNEAL_INITIAL AZK_REWARD_SHAPING_ANNEAL_FINAL \
      AZK_REWARD_SHAPING_ANNEAL_WARMUP_EPISODES AZK_REWARD_SHAPING_ANNEAL_RAMP_EPISODES
draftref combo45

# --- per-checkpoint evidence trajectories (CPU) ---
