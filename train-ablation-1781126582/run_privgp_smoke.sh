#!/bin/bash
# privgp1 smoke arm (15M): full combo recipe + AZK_PORTAL_GP_BONUS=0.3
export AZK_PRIVGP_MARKER=1.
...
mkdir -p "$RESULTS" experiments/runlogs experiments/abl_snapshots/privgp1 experiments/league/privgp1
...
.venv/bin/python python/src/train.py \
  ...
  --env.deck_snapshot_dir experiments/abl_snapshots/privgp1 \
  --league.state_path experiments/league/privgp1/league_state.json \
  --league.opponent_dir experiments/league/privgp1/opponents \
  ...
  > /tmp/train_privgp1.log 2>&1
echo "[portalgp] $(date +%F_%T) train exited rc=$?"

CKPT=$(ls -t experiments/azuki_local_privgp1_*/model_azuki_local_*.pt 2>/dev/null | head -1)
if [ -n "$CKPT" ]; then
  PYTHONPATH=build/python/src:python/src \
  .venv/bin/python python/src/draft_vs_reference_eval.py \
    --config python/config/azuki_deckbuild_native_3090.ini \
    --checkpoint "$CKPT" --episodes 96 --argmax \
    --json "$RESULTS/privgp1_draftref.json" \
    > /tmp/draftref_privgp1.log 2>&1 || true
fi
