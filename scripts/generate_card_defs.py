...
The generated C file contains a table of card definitions and a helper
function that instantiates the cards as Flecs entities with the appropriate
components.
"""
...
TYPE_RULES = {
    "LEADER": {
        "allowed": BASE_REQUIRED_FIELDS | {"health"},
        "required": {"health"},
    },
    "GATE": {
...
def render_c_file(records: Sequence[CardRecord]) -> str:
    ...
    lines.append("static const CardDef kGeneratedCardDefs[] = {")
    for record in records:
        lines.extend(render_card_def_entry(record))
    lines.append("};")
    lines.append("")

    lines.append("static size_t azk_generated_card_defs_count(void) {")
    lines.append("    return sizeof(kGeneratedCardDefs) / sizeof(kGeneratedCardDefs[0]);")
    lines.append("}")
    lines.append("")

    lines.append("static void azk_apply_type_tag(ecs_world_t *world, ecs_entity_t entity, CardType type) {")
    lines.append("    switch (type) {")
    for card_type in TYPE_ORDER:
        tag = TYPE_TAGS[card_type]
        lines.append(f"    case CARD_TYPE_{card_type}:")
        lines.append(f"        ecs_add(world, entity, {tag});")
        lines.append("        break;")
    ...
    lines.append("void azk_register_generated_card_defs(ecs_world_t *world) {")
    lines.append("    const size_t count = azk_generated_card_defs_count();")
    lines.append("    for (size_t index = 0; index < count; ++index) {")
    lines.append("        const CardDef *def = &kGeneratedCardDefs[index];")
    lines.append("        ecs_entity_t card = ecs_entity_init(world, &(ecs_entity_desc_t){")
    lines.append("            .name = def->card_id,")
    lines.append("        });")
    lines.append("")
    lines.append("        if (def->name && def->name[0] != '\\0') {")
    lines.append("            ecs_set_symbol(world, card, def->name);")
    lines.append("        }")
    lines.append("")
    lines.append("        azk_apply_type_tag(world, card, def->type);")
    lines.append("")
    lines.append("        Element element_component = { .element = (uint8_t)def->element };")
    lines.append("        ecs_set_ptr(world, card, Element, &element_component);")
    lines.append("")
    lines.append("        if (def->has_base_stats) {")
    lines.append("            ecs_set_ptr(world, card, BaseStats, &def->base_stats);")
    lines.append("        }")
    lines.append("")
    lines.append("        if (def->has_gate_points) {")
    lines.append("            ecs_set_ptr(world, card, GatePoints, &def->gate_points);")
    lines.append("        }")
    lines.append("")
    lines.append("        if (def->has_ikz_cost) {")
    lines.append("            ecs_set_ptr(world, card, IKZCost, &def->ikz_cost);")
    lines.append("        }")
    lines.append("    }")
    lines.append("}")
    lines.append("")

    lines.append("const CardDef *azk_card_defs_data(size_t *out_count) {")
    lines.append("    if (out_count) {")
    lines.append("        *out_count = azk_generated_card_defs_count();")
    lines.append("    }")
    lines.append("    return kGeneratedCardDefs;")
    lines.append("}")
    lines.append("")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    rendered = render_c_file(records)
    ensure_parent_directory(args.output)
    args.output.write_text(rendered, encoding="utf-8")
    print(f"Wrote {len(records)} card definition(s) to {args.output}")
    return 0
