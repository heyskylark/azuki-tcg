...
ALL_KEYWORDS = set(KEYWORD_TAG_MAPPING.keys()) | set(KEYWORD_WITH_DATA.keys())

TYPE_RULES = {
    "LEADER": {
        "allowed": BASE_REQUIRED_FIELDS | {"attack", "health", "keywords", "subtypes"},
        "required": {"health"},
    },
    "GATE": {
        "allowed": BASE_REQUIRED_FIELDS | {"keywords", "subtypes"},
        "required": set(),
    },
    "ENTITY": {
        "allowed": BASE_REQUIRED_FIELDS | {"attack", "health", "ikz_cost", "gate_points", "keywords", "subtypes"},
        "required": {"attack", "health", "ikz_cost"},
    },
    "WEAPON": {
        "allowed": BASE_REQUIRED_FIELDS | {"attack", "ikz_cost", "keywords", "subtypes"},
        "required": {"attack", "ikz_cost"},
    },
    "SPELL": {
        "allowed": BASE_REQUIRED_FIELDS | {"ikz_cost", "keywords", "subtypes"},
        "required": {"ikz_cost"},
    },
    "IKZ": {
        "allowed": BASE_REQUIRED_FIELDS | {"keywords", "subtypes"},
...
class CardRecord:
    ...
    keywords: tuple  # Tuple of keyword strings (immutable for frozen dataclass)
...
def validate_card_payload(payload: object, lineno: int) -> CardRecord:
    ...
    if not isinstance(keywords_raw, list):
        raise CardValidationError(lineno, "Field 'keywords' must be an array.")

    keywords: List[str] = []
    for kw in keywords_raw:
        if not isinstance(kw, str):
            raise CardValidationError(
                lineno, f"Keyword must be a string, got {type(kw).__name__}"
            )
        kw_lower = kw.lower()
        if kw_lower not in ALL_KEYWORDS:
            allowed = ", ".join(sorted(ALL_KEYWORDS))
            raise CardValidationError(
                lineno, f"Unknown keyword '{kw}'. Expected one of: {allowed}"
            )
...
def render_prefab_component_sets(record: CardRecord, indent: str) -> List[str]:
    ...
    # Add keyword-related components
    for kw in record.keywords:
        if kw in KEYWORD_WITH_DATA:
            kw_data = KEYWORD_WITH_DATA[kw]
            lines.append(
                f"{indent}ecs_set(world, prefab, {kw_data['component']}, {{ {kw_data['init']} }});"
            )
...
def render_prefab_block(record: CardRecord) -> List[str]:
    # Build the add array with base tags + keyword tags
    add_list = ["EcsPrefab", record.prefab_tag]
    for kw in record.keywords:
        if kw in KEYWORD_TAG_MAPPING:
            add_list.append(KEYWORD_TAG_MAPPING[kw])
        elif kw in KEYWORD_WITH_DATA:
            add_list.append(KEYWORD_WITH_DATA[kw]["tag"])
    add_ids = ", ".join(add_list)
...
def render_c_file(records: Sequence[CardRecord], all_subtypes: List[str], header_include: str) -> str:
    ...
    lines.append("#include \"components/abilities.h\"")
    lines.append("")
    for component in CARD_PREFAB_COMPONENT_DECLARATIONS:
