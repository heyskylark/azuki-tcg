...
def tcg_sample_logits(logits, action=None):
    ...
    unit1_logits = _compute_unit_logits(distribution.target_matrix, gate1_weights, distribution.unit1_projection)
    ...
    unit2_logits = _compute_unit_logits(distribution.target_matrix, gate2_weights, distribution.unit2_projection)
    bins2_logits = _pad_or_trim_to_index_dim(distribution.bins2_logits)
    ...
    bins3_logits = _pad_or_trim_to_index_dim(distribution.bins3_logits)
...
def _build_subaction_mask(
    ...
    *,
    ...
    mask = torch.zeros(B, MAX_INDEX_SIZE, device=device, dtype=torch.bool)
    if valid.any():
        target_indexes = legal_actions[..., column].clamp(0, MAX_INDEX_SIZE - 1)
...
def _compute_unit_logits(
    ...
    return _pad_or_trim_to_index_dim(logits)


def _pad_or_trim_to_index_dim(tensor: torch.Tensor) -> torch.Tensor:
    current = tensor.size(-1)
    if current == MAX_INDEX_SIZE:
        return tensor
    if current > MAX_INDEX_SIZE:
        return tensor[..., :MAX_INDEX_SIZE]
    pad_size = MAX_INDEX_SIZE - current
