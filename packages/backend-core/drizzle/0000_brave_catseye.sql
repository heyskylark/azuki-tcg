...
CREATE TYPE "public"."card_rarity" AS ENUM('L', 'G', 'C', 'UC', 'R', 'SR', 'IKZ');--> statement-breakpoint
...
CREATE TABLE "cards" (
	...
	"created_at" timestamp (3) with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp (3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "cards_engine_id_unique" UNIQUE("engine_id"),
...
ALTER TABLE "decks" ADD CONSTRAINT "decks_leader_card_id_cards_id_fk" FOREIGN KEY ("leader_card_id") REFERENCES "public"."cards"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "decks" ADD CONSTRAINT "decks_gate_card_id_cards_id_fk" FOREIGN KEY ("gate_card_id") REFERENCES "public"."cards"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
