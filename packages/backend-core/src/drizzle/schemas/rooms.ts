import { pgTable, text, boolean, integer, timestamp, pgEnum, uuid } from "drizzle-orm/pg-core";
import {
...
export const Rooms = pgTable("rooms", {
  ...
  player0Id: uuid("player0_id").references(() => Users.id),
  player0DeckId: uuid("player0_deck_id").references(() => Decks.id),
  player0Ready: boolean("player0_ready").notNull().default(false),
  player1Id: uuid("player1_id").references(() => Users.id),
  player1DeckId: uuid("player1_deck_id").references(() => Decks.id),
