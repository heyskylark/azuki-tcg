...
static void handle_play_spell_from_hand(ecs_world_t *world, GameState *gs, ActionContext *ac) {
  ...
  if (!azk_validate_play_spell_action(world, gs, player, &ac->user_action, true, &intent)) {
    ac->invalid_action = true;
    return;
  }
...
void HandleResponseAction(ecs_iter_t *it) {
    ...
    switch (ac->user_action.type) {
      case ACT_CONFIRM_ABILITY:
        if (ability_phase == ABILITY_PHASE_CONFIRMATION) {
          ...
          cli_render_logf("[ResponseAction] ACT_CONFIRM_ABILITY not valid in ability phase %d", ability_phase);
          ac->invalid_action = true;
        }
        break;
      ...
      case ACT_NOOP:
        ...
        if (ability_phase == ABILITY_PHASE_CONFIRMATION) {
          ...
          cli_render_logf("[ResponseAction] ACT_NOOP not valid in ability phase %d", ability_phase);
          ac->invalid_action = true;
        }
        break;
      ...
      case ACT_SELECT_COST_TARGET:
        if (ability_phase == ABILITY_PHASE_COST_SELECTION) {
          ...
          cli_render_logf("[ResponseAction] ACT_SELECT_COST_TARGET not valid in ability phase %d", ability_phase);
          ac->invalid_action = true;
        }
        break;
      ...
      case ACT_SELECT_EFFECT_TARGET:
        if (ability_phase == ABILITY_PHASE_EFFECT_SELECTION) {
          ...
          cli_render_logf("[ResponseAction] ACT_SELECT_EFFECT_TARGET not valid in ability phase %d", ability_phase);
          ac->invalid_action = true;
        }
        break;

      default:
        cli_render_logf("[ResponseAction] Action type %d not valid during ability phase", ac->user_action.type);
        ac->invalid_action = true;
        break;
    }
    return;
  ...
  switch (ac->user_action.type) {
    case ACT_PLAY_SPELL_FROM_HAND:
      ...
      // Stay in response window after spell resolves (allows multiple spells)
      break;
    ...
    default:
      cli_render_logf("[ResponseAction] Unknown response action type: %d", ac->user_action.type);
      ac->invalid_action = true;
      break;
  }
}
