...
void DrawCard(ecs_iter_t *it) {
  ...
  ecs_iter_t cards_in_deck =
    get_cards_owned_by_player_in_zone(world, player, deck_zone);

  // TODO: Why query not returning anything
  printf("Cards in deck: %d\n", cards_in_deck.count);

  if (!ecs_query_next(&cards_in_deck) || cards_in_deck.count == 0) {
    printf("No cards in deck (%d)\n", cards_in_deck.count);
    // TODO: Player loses
    return;
  }
...
void GrantIKZ(ecs_iter_t *it) {
  ...
  ecs_iter_t cards_in_ikz_pile =
    get_cards_owned_by_player_in_zone(world, player, ikz_pile_zone);
