...
static void update_jd_buff(ecs_world_t *world, ecs_entity_t card,
                           uint8_t player_num) {
  ...
  if (owner_garden_has_only_normal_entities(world, player_num)) {
    azk_queue_passive_buff_update(world, card, card, 2, 0, false);
  } else {
    azk_queue_passive_buff_update(world, card, card, 0, 0, true);
  }
}
...
void azk01_010_init_passive_observers(ecs_world_t *world,
  ...
  Azk01010ObserverCtx *ctx =
      azk_alloc_passive_observer_ctx(sizeof(Azk01010ObserverCtx));
  if (!ctx) {
    return;
  }
  ...
  ecs_entity_t garden_observer = azk_create_tracked_passive_observer(
      ...
      &(ecs_observer_desc_t){
          ...
          .callback = azk01_010_zone_observer,
          .ctx = ctx,
      });
  ...
  update_jd_buff(world, card, ctx->player_num);
}
