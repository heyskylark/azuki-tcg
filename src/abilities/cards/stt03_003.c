...
static bool is_verdant_selection_card(ecs_world_t *world, ecs_entity_t card,
                                      const void *user_ctx) {
  (void)user_ctx;
  return has_subtype(world, card, ecs_id(TSubtype_Verdant));
}
...
void stt03_003_on_cost_paid(ecs_world_t *world, AbilityContext *ctx) {
  azk_setup_reveal_top_cards_selection(world, ctx, 5, 1,
                                       is_verdant_selection_card, NULL);
}
...
bool stt03_003_validate_selection_target(ecs_world_t *world, ecs_entity_t card,
  ...
  (void)card;
  (void)owner;
  return target != 0 && is_verdant_selection_card(world, target, NULL);
}
