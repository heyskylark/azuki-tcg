...
static void sync_geodust_smuggler_buffs(ecs_world_t *world, ecs_entity_t card,
                                        uint8_t owner_player_num) {
  ...
  bool active =
      ecs_get_target(world, card, EcsChildOf, 0) == owner_garden;
  ...
  for (int32_t i = 0; i < garden_cards.count; ++i) {
    ...
    if (entity == card || !ecs_has_id(world, entity, TEntity)) {
      continue;
    }

    bool has_buff = ecs_has_pair(world, entity, ecs_id(HealthBuff), card);
    if (active && !has_buff) {
      azk_queue_passive_buff_update(world, entity, card, 0, 1, false);
    } else if (!active && has_buff) {
      azk_queue_passive_buff_update(world, entity, card, 0, 0, true);
    }
  }
}
...
void azk01_053_init_passive_observers(ecs_world_t *world, ecs_entity_t card) {
  ...
  const GameState *gs = ecs_singleton_get(world, GameState);
  Azk01053ObserverCtx *ctx =
      azk_alloc_passive_observer_ctx(sizeof(Azk01053ObserverCtx));
  if (ctx == NULL) {
    return;
  }
  ...
  ecs_entity_t observer = azk_create_tracked_passive_observer(
      ...
      &(ecs_observer_desc_t){
          ...
          .callback = azk01_053_garden_observer,
          .ctx = ctx,
      });
