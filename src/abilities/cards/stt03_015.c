...
bool stt03_015_validate(ecs_world_t *world, ecs_entity_t card,
                        ecs_entity_t owner) {
  (void)world;
  (void)card;
  (void)owner;
...
void stt03_015_apply_effects(ecs_world_t *world, const AbilityContext *ctx) {
  const GameState *gs = ecs_singleton_get(world, GameState);
  const uint8_t owner_num = get_player_number(world, ctx->runtime.owner);
  const int8_t heal_amount =
      ecs_get_ordered_children(world, gs->zones[owner_num].ikz_area).count >= 7 ? 5 : 3;
  heal_owner_leader(world, ctx->runtime.owner, heal_amount);
}
