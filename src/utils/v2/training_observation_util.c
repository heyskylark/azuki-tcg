...
static TrainingPreviousActionObservationData
build_previous_action_observation(ecs_world_t *world, const GameState *gs,
                                  int8_t player_index) {
  TrainingPreviousActionObservationData observation = {0};
  ...
  for (uint32_t offset = 0; offset < history_count; ++offset) {
    const uint32_t history_index =
        (action_context->history_head + MAX_USER_ACTION_HISTORY_SIZE - 1u -
         offset) %
        MAX_USER_ACTION_HISTORY_SIZE;
    const UserAction *action = &action_context->user_action_history[history_index];
    ...
    observation.has_action = true;
    observation.primary = (uint8_t)action->type;
    observation.sub1 = clamp_subaction_to_u8(action->subaction_1);
    observation.sub2 = clamp_subaction_to_u8(action->subaction_2);
    observation.sub3 = clamp_subaction_to_u8(action->subaction_3);
    observation.was_noop = action->type == ACT_NOOP;
    return observation;
  }
...
TrainingObservationData create_training_observation_data(ecs_world_t *world,
  ...
  observation.previous_action =
      build_previous_action_observation(world, gs, player_index);
